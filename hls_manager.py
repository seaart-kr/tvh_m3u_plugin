...
class HLSManager:
            ...
            with cls._lock:
                for channel_uuid, state in list(cls._streams.items()):
                    ...
                    if is_dead or is_idle:
                        stale.append(cls._streams.pop(channel_uuid))
            for state in stale:
    ...
    @classmethod
    def ensure_stream(cls, channel_uuid, upstream_url, username='', password=''):
        ...
        with cls._lock:
            ...
            if state and state.get('process') and state['process'].poll() is None:
                ...
                if state:
                    cls._streams.pop(channel_uuid, None)
                    cls._stop_state(state, clear=True)
    ...
    @classmethod
    def get_playlist(cls, channel_uuid):
        channel_uuid = str(channel_uuid or '').strip()
    ...
    @classmethod
    def touch(cls, channel_uuid):
        with cls._lock:
            state = cls._streams.get(str(channel_uuid or '').strip())
            if state:
    ...
    @classmethod
    def read_segment(cls, channel_uuid, filename):
        channel_uuid = str(channel_uuid or '').strip()
