...
class Task(TaskConnection, TaskSync, TaskGroup, TaskSheet, TaskProfile, TaskM3U):
        ...
        try:
            ...
            for row in rows:
                ...
                if table_name in ['alembic_version']:
                    continue
                if 'setting' in table_name.lower():
                    continue
            ...
            logger.info(f"[ff_tvh_m3u] reset_plugin_db success tables={deleted_tables}")
            return {
                'ret': 'success',
    ...
    def search_master_channels(keyword, limit=30):
        ...
        try:
            ...
            logger.info(
                f'[ff_tvh_m3u] search_master_channels keyword={keyword!r} results={len(items)} '
                f'sample={[item.get("standard_name", "") for item in items[:5]]}'
            )
            ...
            logger.exception(f'[ff_tvh_m3u] search_master_channels exception: {str(e)}')
            return {
                'ret': 'danger',
    ...
    def add_db_match_channel(channel_uuid, channel_id):
        ...
        try:
            ...
            if exists is not None:
                con.close()
                logger.info(
                    f'[ff_tvh_m3u] add_db_match_channel already_exists '
                    f'channel_uuid={channel_uuid} channel={alias_name!r} '
                    f'master_id={channel_id} master={str(master["standard_name"] or "")!r} '
                    f'group={str(master["group_category"] or "")!r}'
                )
                return {
                    'ret': 'warning',
            ...
            logger.info(
                f'[ff_tvh_m3u] add_db_match_channel success '
                f'channel_uuid={channel_uuid} channel={alias_name!r} '
                f'master_id={channel_id} master={str(master["standard_name"] or "")!r} '
                f'group={str(master["group_category"] or "")!r}'
            )
            ...
            logger.exception(f'[ff_tvh_m3u] add_db_match_channel exception: {str(e)}')
            return {
                'ret': 'danger',
